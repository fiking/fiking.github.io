...
第二，指针的位置以什么粒度记录？最准确的方式是记录域的地址到指针被写的地方。但是如果一个对象有许多的域则这会使得记录集的大小增加，如一个数组被更新。一个可选的方式是记录包含这个域的对象地址：这也允许需要过滤的冗余，而域记录则不会有（因为域里通常没有空间记录已经被记录的）。对象记录需要回收器在扫描期间扫描对象里的每个指针，从而发现那些被称为对象需要被记录。假设如果一个数组的一个槽被更新，则很多槽可能也会被更新，则一个混合的方案可能是对象记录数组和域记录标量。相反的是，域记录数组（避免扫描整个对象）和对象记录标量（因为它们倾向于更小）可能是明智的。对于数组，只记录数组的一部分可能是有意义的。这类似类似于卡标记，但是只特定于数组和数组的索引对齐，而不是与虚拟内存中的数组域的地址。是否存储对象或它槽中的一个可能也依赖于修改器持有的信息。如果写动作知道对象地址和域，屏障可以选择记录任意一个；如果只有域地址被传递给屏障，则计算对象的地址会导致更高的固有开销。Hosking等人【1992】解决了这个困境，在一个解释性的Smalltalk系统中，将对象和槽的地址存储在它们的顺序存储区间里。Blackburn【2019】给出了域记录的设计方案的性能数据，其使用了每个域一位的方式来避免多次记录一个域。这些位存储在与该对象相邻的位置。这个工作表明一个小心的实现会使得域记录的固有开销比起对象记录的固有开销会降低1%到2%。

卡表技术（我们下面将会讨论的）将堆逻辑上分为小的，固定大小的卡。指针修改以卡表为粒度进行记录，通常是设置一个字节到一个卡表里。注意到卡标记可以对应到一个域或者一个对象的更新（它们可能在不同的卡片上）。在回收期间里，回收器首先必须寻找到所有待回收代对应的脏卡，然后寻找出卡中记录的所有感兴趣的指针。对象卡标记还是域卡标记的选择会影响到如何进行搜索。指针存储区的粒度比卡片更粗，它按虚拟内存页的粒度进行记录。通过硬件和操作系统的支持，这可能不会在修改器上引入直接的开销，但是类似于卡表，会增加回收器的工作。不像卡表，标记页可能总是会对应到更新的槽而不是它包含的对象，因为操作系统无法观察到对象布局。

第三，记录集是否允许包含重复的项？不检查冗余的情况下可以减轻修改器的负担；但是相应的冗余会增加记录集的大小，会将处理冗余的开销移动到回收器上。卡和页标记消除了冗余，因为它们通常简化为设置表中的一位或者一个字节。对象记录还可以通过标记对象已记录来消除大部分重复项，例如通过使用它们头上的一位，而不管记录集本身如何实现，然而如果有字大小的域被记录（但是Zhao等人【2022a,b】通过域记录取得了一个好的结果），则重复项消除未必会这么简单。修改器的成本是有一个额外的检查，这个额外检查能否随着记录集条目的减少而减少尚不确定，并且还要执行一个写操作。然而，如果不包含重复项，则记录集必须实现为真集而不是多集。

总之，如果使用基于卡表或者页的方案，则回收器扫描成本取决于脏卡或者脏页的数量。此外，如果一个方案没有使用冗余消除，则成本取决于指针写的数量。有冗余消除，则它取决于不同对象修改的数量。在所有的场景里，不感兴趣指针过滤将会降低回收器根扫描成本。实现记录集的机制包含哈希集，顺序存储空间，卡表，虚拟内存和硬件支持。我们将会依次考虑每一项。

### 哈希表

如果记录集记录的槽没有冗余项，则它必须实现为真正的集合。 等价的，如果有对象头里没有空间来标记一个对象已经记录了，则对象记录也需要一个集合。一个记录集未来需要的是能够快速地添加一项，最好是常量时间的操作。哈希表满足这些需求。

Hosking等人【1992】通过一个环形哈希表为Smalltalk解释器实现了一个记录集，在他们的多代内存管理工具包里使用了线性哈希，这个解释器存储栈帧在堆的0代0步里。更特别的是，每代都维护了一个独立的记录集。它们的记录集可以存储每个对象或者域。这个表实现为一个具有$2^i + k$项的数组（它们使用$k = 2$）。然后地址被哈希为获得i位（来自地址的中间位），然后哈希被用于索引数组。如果条目为空，则对象或者域的地址被存储到这个索引里，否则，搜索下面k项（此操作并非循环执行的，这就是为啥数组大小为$2^i + k$）。如果这个查找空项依旧是失败的，则循环搜索这个表。

为了避免进一步增加，必须由记录代码完成这个工作，写屏障过滤出所有到0代的写和所有年轻代的写。此外，它将所有的感兴趣的指针添加到一个单次擦除式记忆集而不是目标分代的记忆集。在修改器运行期间不预先确定添加到哪个分代的记忆集中，这种做法在多线程环境下可能更为恰当；可以使用每个处理器擦除记忆集来避免争用，因为线程安全的哈希表开销过高。总之，Hosking等人在写屏障的快速路径里使用了17条内联MIPS指令，包括了更新记忆集的调用，即便在MIPS架构（一个富寄存器架构）上，这也使得其相对昂贵。在回收期间，一个给定分代的根可能会存在于分代的记忆集或者擦除记忆集上。记忆集间的冗余会被移除，通过将分代记忆集哈希到擦除记忆集上，且擦除集会被做如下：任意遇到的感兴趣的指针会被添加到合适的记忆集上。在10.4节我们会看到OpenJDK G1回收器同样不会在修改器运行期间决定哪个区域的记忆集会被更新，但是只有在需要的时候，它的写屏障会简单地更新一个追加到线程局部的顺序存储区间的卡表（见下面）；
